...
const NetworkGraph = () => {
  ...
  const processFlowData = () => {
    // Extract unique IP addresses (nodes)
    ...
    // Create links array (connections between IPs)
    const links = recentFlows.map(flow => ({
      source: flow.src_ip,
      target: flow.dst_ip,
      value: flow.bytes_sent + flow.bytes_received,
  ...
  useEffect(() => {
    if (!recentFlows.length || !svgRef.current) return;
    ...
    const protocolColor = (protocol) => {
      switch(protocol) {
        case 'TCP': return "#4299e1";  // blue
        case 'UDP': return "#48bb78";  // green
        case 'ICMP': return "#ed8936"; // orange
    ...
    <div className="p-6">
      ...
      <div className="bg-white p-4 rounded-lg shadow-md mb-4">
        <div className="flex justify-between items-center mb-4">
          <div>
            <span className="font-medium">Total Nodes:</span> {processFlowData().nodes.length}
            <span className="ml-4 font-medium">Total Connections:</span> {processFlowData().links.length}
          </div>
          <div>
